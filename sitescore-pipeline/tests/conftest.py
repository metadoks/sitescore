...
def engine(): return build_geometry_engine_identity()

@pytest.fixture(scope="session")
def crs3857(): return build_crs_identity("EPSG:3857")

@pytest.fixture(scope="session")
def canon_policy(): return GeometryCanonicalizationPolicy()

@pytest.fixture(scope="session")
def geography(): return GeographyIdentity("BENCHMARK_AREA", "area-1", "boundary-def-2026")

def _boundary(geo, crs, engine, canon_policy):
    geometry = canonicalize_geometry(
        Polygon([(-100,-100),(300,-100),(300,300),(-100,300),(-100,-100)]),
        crs_identity=crs, policy=canon_policy, engine=engine,
    )
    source = build_geometry_source_identity(
        provider="fixture", dataset="boundary", release="2026-08-01",
        vintage="2026", schema_version="1", source_crs_identity=crs,
        raw_content=b"pipeline-boundary",
    )
    return build_boundary_geometry_artifact(
        geography_identity=geo, geometry_role="BENCHMARK_BOUNDARY",
        source_identity=source, canonical_geometry=geometry, parser_id="fixture",
        parser_version="1", canonicalization_policy=canon_policy, engine=engine,
...
def evidence_policy():
    return CommercialEvidencePolicy(
        "commercial-evidence", "1.0", ("qualifying_place",),
...
def unsafe_resolved_lattice(crs, canon_policy, engine, *, size=100.0, anchor=(0.0,0.0), version="test"):
    return LatticePolicy(
        "test-lattice", version, unsafe_resolved_projection(crs),
        CellResolutionPolicy("test-resolution", version, ResolutionState.RESOLVED, size, "m"),
        CellShape.SQUARE, anchor[0], anchor[1], "AXIS_ALIGNED", "INTEGER_IJ",
        "1.0", canon_policy, engine,
    )

def make_area_result(geom, canon_policy, engine):
    from sitescore_spatial import AreaPolicy, GeometryOperation, GeometryOperationPolicy, area_of_projected_geometry
    return area_of_projected_geometry(
        geom, area_policy=AreaPolicy(),
        operation_policy=GeometryOperationPolicy(
            "cell-area", "1.0", GeometryOperation.AREA, canon_policy.precision_policy
        ),
        engine=engine,
    )
