"""Immutable road network/routing policy and evidence contracts (Checkpoint 3.3-7)."""
...
ROAD_NETWORK_MANIFEST_GRAMMAR = "v1"
ROAD_ROUTING_ENGINE_MANIFEST_GRAMMAR = "v1"
ROAD_TRAFFIC_POLICY_GRAMMAR = "v1"
ROAD_GRAPH_COMPATIBILITY_GRAMMAR = "v1"
ROAD_DRIVE_BUDGET_POLICY_GRAMMAR = "v1"
ROAD_GEOMETRY_POLICY_GRAMMAR = "v1"
ROAD_AREA_POLICY_GRAMMAR = "v1"
ROAD_AREA_EVIDENCE_GRAMMAR = "v1"
ROAD_MEASUREMENT_POLICY_GRAMMAR = "v1"
ROAD_ORIGIN_GRAMMAR = "v1"
ROAD_REQUEST_GRAMMAR = "v1"
ROAD_GEOMETRY_GRAMMAR = "v1"
ROAD_EVIDENCE_GRAMMAR = "v1"
ROAD_DERIVATION_GRAMMAR = "v1"
ROAD_EXECUTION_POLICY_GRAMMAR = "v1"
ROAD_EXECUTION_BINDING_GRAMMAR = "v1"
ROAD_VALHALLA_WARNING_GRAMMAR = "v1"

VALHALLA_PROVIDER_KEY = "valhalla"
VALHALLA_ISOCHRONE_DATASET = "valhalla_isochrone"
VALHALLA_ISOCHRONE_OPERATION = "road_isochrone"
VALHALLA_PARSER_ID = "valhalla_isochrone_geojson"
ROAD_TRAVEL_MODE = "drive"
...
class RoadNetworkManifest:
    ...
    def identity(self) -> ContentHash:
        return hash_canonical({
            "grammar_version": ROAD_NETWORK_MANIFEST_GRAMMAR,
...
@dataclass(frozen=True, slots=True)
class RoadTrafficPolicy:
    policy_id: str
    policy_version: str
    traffic_semantics: str = "static_graph_no_datetime"
    ...
    def __post_init__(self) -> None:
        ...
        if self.traffic_semantics != "static_graph_no_datetime":
            raise ValueError("canonical V1 road evidence requires static_graph_no_datetime traffic semantics")

    @property
    def identity(self) -> ContentHash:
        return hash_canonical({
            "grammar_version": ROAD_TRAFFIC_POLICY_GRAMMAR,
            "canonicalization_version": CANONICALIZATION_VERSION,
            "policy_id": self.policy_id,
            "policy_version": self.policy_version,
            "traffic_semantics": self.traffic_semantics,
        })


@dataclass(frozen=True, slots=True)
class RoadRoutingEngineManifest:
    manifest_version: str
    engine_id: str
    engine_version: str
    ...
    traffic_policy: RoadTrafficPolicy
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.traffic_policy, RoadTrafficPolicy):
            raise TypeError("traffic_policy must be a RoadTrafficPolicy")
        require_nonempty_text(self.request_grammar_version, field_name="request_grammar_version")

    @property
    def identity(self) -> ContentHash:
        return hash_canonical({
            "grammar_version": ROAD_ROUTING_ENGINE_MANIFEST_GRAMMAR,
            "canonicalization_version": CANONICALIZATION_VERSION,
            "manifest_version": self.manifest_version,
            ...
            "traffic_policy_identity": str(self.traffic_policy.identity),
            "request_grammar_version": self.request_grammar_version,
        })


@dataclass(frozen=True, slots=True)
class RoadGraphCompatibility:
    ...
    network_manifest: RoadNetworkManifest
    routing_engine_manifest: RoadRoutingEngineManifest
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.network_manifest, RoadNetworkManifest):
            raise TypeError("network_manifest must be a RoadNetworkManifest")
        if not isinstance(self.routing_engine_manifest, RoadRoutingEngineManifest):
            raise TypeError("routing_engine_manifest must be a RoadRoutingEngineManifest")
    ...
    def identity(self) -> ContentHash:
        return hash_canonical({
            "grammar_version": ROAD_GRAPH_COMPATIBILITY_GRAMMAR,
            "canonicalization_version": CANONICALIZATION_VERSION,
...
@dataclass(frozen=True, slots=True)
class RoadIsochroneExecutionPolicy:
    ...
    def identity(self) -> ContentHash:
        return hash_canonical({
            "grammar_version": ROAD_EXECUTION_POLICY_GRAMMAR,
            "canonicalization_version": CANONICALIZATION_VERSION,
            "policy_id": self.policy_id,
            "policy_version": self.policy_version,
...
@dataclass(frozen=True, slots=True)
class RoadRoutingExecutionBinding:
    ...
    binding_id: str
    binding_version: str
    graph_compatibility: RoadGraphCompatibility
    execution_policy: RoadIsochroneExecutionPolicy
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.graph_compatibility, RoadGraphCompatibility):
            raise TypeError("graph_compatibility must be a RoadGraphCompatibility")
        if not isinstance(self.execution_policy, RoadIsochroneExecutionPolicy):
            raise TypeError("execution_policy must be a RoadIsochroneExecutionPolicy")

    @property
    def identity(self) -> ContentHash:
        return hash_canonical({
            "grammar_version": ROAD_EXECUTION_BINDING_GRAMMAR,
            "canonicalization_version": CANONICALIZATION_VERSION,
...
class RoadValhallaWarningState(StrEnum):
    UNRESOLVED = "unresolved"


@dataclass(frozen=True, slots=True)
class RoadValhallaWarningEvidence:
    ...
    state: RoadValhallaWarningState = RoadValhallaWarningState.UNRESOLVED

    def __post_init__(self) -> None:
        if not isinstance(self.state, RoadValhallaWarningState):
            raise TypeError("state must be a RoadValhallaWarningState")
    ...
    def identity(self) -> ContentHash:
        return hash_canonical({
            "grammar_version": ROAD_VALHALLA_WARNING_GRAMMAR,
            "canonicalization_version": CANONICALIZATION_VERSION,
...
@dataclass(frozen=True, slots=True)
class RoadDriveBudgetScale:
    scale_id: str
    travel_cost_seconds: float
    travel_cost_unit: str = "seconds"
    ...
    def __post_init__(self) -> None:
        ...
        if self.travel_cost_unit != "seconds":
            raise ValueError('canonical V1 road travel_cost_unit must be "seconds"')


@dataclass(frozen=True, slots=True)
class RoadDriveBudgetPolicy:
    policy_id: str
    policy_version: str
    scales: tuple[RoadDriveBudgetScale, ...]
    ...
    def __post_init__(self) -> None:
        ...
        if not self.scales:
            raise ValueError("driving budget policy requires at least one scale")
        for scale in self.scales:
            if not isinstance(scale, RoadDriveBudgetScale):
                raise TypeError("scales must contain RoadDriveBudgetScale values")
        ...
        if len(ids) != len(set(ids)):
            raise ValueError("driving budget scale_id values must be unique")
        if len(costs) != len(set(costs)):
            raise ValueError("driving budget travel costs must be unique")
        expected = tuple(sorted(self.scales, key=lambda s: (float(s.travel_cost_seconds), s.scale_id)))
        if self.scales != expected:
            raise ValueError("driving budget scales must be deterministically ordered by travel cost then scale_id")

    @property
    def identity(self) -> ContentHash:
        return hash_canonical({
            "grammar_version": ROAD_DRIVE_BUDGET_POLICY_GRAMMAR,
            "canonicalization_version": CANONICALIZATION_VERSION,
            "policy_id": self.policy_id,
            "policy_version": self.policy_version,
            "scales": tuple({"scale_id": s.scale_id, "travel_cost_seconds": float(s.travel_cost_seconds), "travel_cost_unit": s.travel_cost_unit} for s in self.scales),
        })


@dataclass(frozen=True, slots=True)
class RoadGeometryPolicy:
    policy_id: str
    policy_version: str
    crs_id: str
    representation: str
    ...
    def __post_init__(self) -> None:
        ...
        if not self.polygons:
            raise ValueError("canonical road reachability requires polygon output")
    ...
    def identity(self) -> ContentHash:
        return hash_canonical({
            "grammar_version": ROAD_GEOMETRY_POLICY_GRAMMAR,
            "canonicalization_version": CANONICALIZATION_VERSION,
            "policy_id": self.policy_id,
            "policy_version": self.policy_version,
            "crs_id": self.crs_id,
...
class RoadAreaPolicy:
    ...
    def __post_init__(self) -> None:
        ...
        if self.output_unit != "km2":
            raise ValueError('road area policy output_unit must be "km2"')

    @property
    def identity(self) -> ContentHash:
        return hash_canonical({
            "grammar_version": ROAD_AREA_POLICY_GRAMMAR,
            "canonicalization_version": CANONICALIZATION_VERSION,
            "policy_id": self.policy_id,
            "policy_version": self.policy_version,
...
class RoadRoutingOrigin:
    ...
    @classmethod
    def from_resolved_location(cls, location: ResolvedLocation) -> "RoadRoutingOrigin":
    ...
    def identity(self) -> ContentHash:
        return hash_canonical({
            "grammar_version": ROAD_ORIGIN_GRAMMAR,
            "canonicalization_version": CANONICALIZATION_VERSION,
...
@dataclass(frozen=True, slots=True)
class RoadIsochroneRequest:
    origin: RoadRoutingOrigin
    budget_policy: RoadDriveBudgetPolicy
    graph_compatibility: RoadGraphCompatibility
    execution_binding: RoadRoutingExecutionBinding
    geometry_policy: RoadGeometryPolicy

    def __post_init__(self) -> None:
        if not isinstance(self.origin, RoadRoutingOrigin):
            raise TypeError("origin must be a RoadRoutingOrigin")
        if not isinstance(self.budget_policy, RoadDriveBudgetPolicy):
            raise TypeError("budget_policy must be a RoadDriveBudgetPolicy")
        if not isinstance(self.graph_compatibility, RoadGraphCompatibility):
            raise TypeError("graph_compatibility must be a RoadGraphCompatibility")
        if not isinstance(self.execution_binding, RoadRoutingExecutionBinding):
        ...
        if not isinstance(self.geometry_policy, RoadGeometryPolicy):
            raise TypeError("geometry_policy must be a RoadGeometryPolicy")
        if self.engine_manifest.costing_profile_id != "auto":
            raise ValueError('canonical V1 road request requires costing_profile_id="auto"')
        if self.geometry_policy.representation != "geojson":
            raise ValueError('canonical V1 road request requires geometry representation "geojson"')
        if self.geometry_policy.crs_id != "epsg_4326":
            raise ValueError('canonical V1 road request requires WGS84 GeoJSON coordinates (epsg_4326)')
        execution = self.execution_binding.execution_policy
        if len(self.budget_policy.scales) > execution.max_contours:
            raise ValueError("driving budget exceeds deployed Valhalla max_contours")
        if any(float(scale.travel_cost_seconds) / 60.0 > float(execution.max_time_contour_minutes) for scale in self.budget_policy.scales):
            raise ValueError("driving budget exceeds deployed Valhalla max_time_contour_minutes")

    @property
    def engine_manifest(self) -> RoadRoutingEngineManifest:
    ...
    def semantic_parameters(self) -> dict[str, object]:
        ...
        return {
            ...
            "contours": tuple({"travel_cost_seconds": float(s.travel_cost_seconds), "travel_cost_unit": s.travel_cost_unit} for s in self.budget_policy.scales),
            ...
            "traffic_policy_identity": str(engine.traffic_policy.identity),
    ...
    def __post_init__(self) -> None:
        ...
        if self.snap_state is OriginSnapState.RESOLVED:
            if self.road_origin_quality is RoadOriginQuality.UNRESOLVED:
                raise ValueError("RESOLVED snap requires resolved road_origin_quality")
            ...
            if self.road_origin_quality is not RoadOriginQuality.UNRESOLVED:
                raise ValueError("non-RESOLVED snap requires UNRESOLVED road_origin_quality")
...
class CanonicalRoadGeometry:
    ...
    def identity(self) -> ContentHash:
        return hash_canonical({
            "grammar_version": ROAD_GEOMETRY_GRAMMAR,
            "canonicalization_version": CANONICALIZATION_VERSION,
...
@dataclass(frozen=True, slots=True)
class RoadContourEvidence:
    scale_id: str
    travel_cost_seconds: float
    geometry: CanonicalRoadGeometry
    ...
    def __post_init__(self) -> None:
        ...
        if float(self.travel_cost_seconds) <= 0:
            raise ValueError("travel_cost_seconds must be > 0")
        if not isinstance(self.geometry, CanonicalRoadGeometry):
            raise TypeError("geometry must be CanonicalRoadGeometry")


@dataclass(frozen=True, slots=True)
class RoadIsochroneEvidence:
    request_fingerprint: RequestFingerprint
    request: RoadIsochroneRequest
    routed_origin: RoutedOriginEvidence
    contours: tuple[RoadContourEvidence, ...]
    warnings: tuple[RoadValhallaWarningEvidence, ...]
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.request, RoadIsochroneRequest):
        ...
        for contour in self.contours:
            if not isinstance(contour, RoadContourEvidence):
                raise TypeError("contours must contain RoadContourEvidence values")
        _require_tuple(self.warnings, field_name="warnings")
        for warning in self.warnings:
            if not isinstance(warning, RoadValhallaWarningEvidence):
                raise TypeError("warnings must contain RoadValhallaWarningEvidence values")
        ...
        if actual_scales != expected_scales:
            raise ValueError("contours must exactly match requested driving budget scales")

    @property
    def identity(self) -> ContentHash:
        return hash_canonical({
            "grammar_version": ROAD_EVIDENCE_GRAMMAR,
            "canonicalization_version": CANONICALIZATION_VERSION,
            ...
            "routed_origin": {
                ...
                "road_origin_quality": self.routed_origin.road_origin_quality.value,
            },
            "contours": tuple({
                "scale_id": c.scale_id,
...
@dataclass(frozen=True, slots=True)
class RoadAreaEvidence:
    scale_id: str
    geometry_identity: ContentHash
    area_km2: float
    area_policy: RoadAreaPolicy
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.area_policy, RoadAreaPolicy):
            raise TypeError("area_policy must be a RoadAreaPolicy")

    @property
    def identity(self) -> ContentHash:
        return hash_canonical({
            "grammar_version": ROAD_AREA_EVIDENCE_GRAMMAR,
            "canonicalization_version": CANONICALIZATION_VERSION,
...
@dataclass(frozen=True, slots=True)
class RoadScaleMeasurementEvidence:
    scale_id: str
    geometry_identity: ContentHash
    ...
    area_policy: RoadAreaPolicy
    network_measurement_method_id: str
    network_measurement_method_version: str

    def __post_init__(self) -> None:
        require_canonical_id(self.scale_id, field_name="scale_id")
        if not isinstance(self.geometry_identity, ContentHash):
            raise TypeError("geometry_identity must be a ContentHash")
        for name in ("reachable_area_km2", "reachable_network_length_km", "origin_to_network_cost_seconds"):
            value = getattr(self, name)
            require_finite_number(value, field_name=name)
            if float(value) < 0:
                raise ValueError(f"{name} must be >= 0")
        if isinstance(self.reachable_connector_count, bool) or not isinstance(self.reachable_connector_count, int):
            raise TypeError("reachable_connector_count must be an int")
        if self.reachable_connector_count < 0:
            raise ValueError("reachable_connector_count must be >= 0")
        if not isinstance(self.area_policy, RoadAreaPolicy):
            raise TypeError("area_policy must be a RoadAreaPolicy")
        require_canonical_id(self.network_measurement_method_id, field_name="network_measurement_method_id")
        require_nonempty_text(self.network_measurement_method_version, field_name="network_measurement_method_version")

    @property
    def identity(self) -> ContentHash:
        return hash_canonical({
            "grammar_version": "road_scale_measurement.v1",
            "canonicalization_version": CANONICALIZATION_VERSION,
            "scale_id": self.scale_id,
            "geometry_identity": str(self.geometry_identity),
            "reachable_area_km2": float(self.reachable_area_km2),
            "reachable_network_length_km": float(self.reachable_network_length_km),
            "reachable_connector_count": self.reachable_connector_count,
            "origin_to_network_cost_seconds": float(self.origin_to_network_cost_seconds),
            "area_policy_identity": str(self.area_policy.identity),
            "network_measurement_method_id": self.network_measurement_method_id,
            "network_measurement_method_version": self.network_measurement_method_version,
        })


@dataclass(frozen=True, slots=True)
class RoadDerivationEvidence:
    isochrone_evidence_identity: ContentHash
    measurement_policy_identity: ContentHash
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.measurement_policy_identity, ContentHash):
            raise TypeError("measurement_policy_identity must be a ContentHash")
        ...
        _require_tuple(self.measurement_evidence_ids, field_name="measurement_evidence_ids")
        for ref in self.measurement_evidence_ids:
            require_canonical_id(ref, field_name="measurement_evidence_id")
        require_canonical_id(self.snapshot_id, field_name="snapshot_id")

    @property
    def identity(self) -> ContentHash:
        return hash_canonical({
            "grammar_version": ROAD_DERIVATION_GRAMMAR,
            "canonicalization_version": CANONICALIZATION_VERSION,
            "isochrone_evidence_identity": str(self.isochrone_evidence_identity),
            "measurement_policy_identity": str(self.measurement_policy_identity),
            "network_source_ref": self.network_source_ref,
            "routing_source_ref": self.routing_source_ref,
            "measurement_evidence_ids": self.measurement_evidence_ids,
