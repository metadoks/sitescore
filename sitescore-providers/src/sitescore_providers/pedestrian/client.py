"""Valhalla pedestrian isochrone acquisition over a minimal JSON POST boundary.
...
VALHALLA_REQUEST_POLICY_ID = "valhalla_pedestrian_isochrone_request"
...
def routing_provider_identity(request: PedestrianIsochroneRequest) -> ProviderIdentity:
...
def build_isochrone_request_fingerprint(request: PedestrianIsochroneRequest) -> RequestFingerprint:
    if not isinstance(request, PedestrianIsochroneRequest):
        raise TypeError("request must be a PedestrianIsochroneRequest")
    engine = request.engine_manifest
    return build_request_fingerprint(
        ...
        grammar_version=PEDESTRIAN_REQUEST_GRAMMAR,
    )


def build_valhalla_isochrone_body(request: PedestrianIsochroneRequest) -> bytes:
    ...
    if engine.costing_options:
        payload["costing_options"] = {
            "pedestrian": {key: value for key, value in engine.costing_options}
        }
...
class PedestrianIsochroneClient:
    def __init__(
        self,
        *,
        ...
        if not isinstance(execution_binding, ValhallaExecutionBinding):
    ...
    def acquire(
        ...
        *,
        ...
        if status == 400:
            ...
            # condition (no suitable pedestrian edge near the input), not zero reach.
            try:
    ...
    def parse_success(
        ...
        *,
        ...
        if raw_artifact.request_fingerprint != expected_fingerprint:
            raise ValueError("raw artifact request fingerprint does not match active pedestrian request")
        if raw_artifact.provider_identity != routing_provider_identity(request):
            raise ValueError("raw artifact provider identity does not match active pedestrian request")
