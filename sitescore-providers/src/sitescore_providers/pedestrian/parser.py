"""Strict Valhalla GeoJSON parsing and deterministic pedestrian geometry canonicalization."""
...
def canonicalize_geometry(
    *, geometry: object, request: PedestrianIsochroneRequest
) -> CanonicalPedestrianGeometry:
    ...
    return CanonicalPedestrianGeometry(
...
def _find_scale(request: PedestrianIsochroneRequest, contour_minutes: float):
    ...
    if len(matches) != 1:
        raise ProviderInvariantError("returned contour does not match exactly one requested walking scale")
    return matches[0]
...
def validate_pedestrian_isochrone_evidence(evidence: PedestrianIsochroneEvidence) -> None:
    ...
    if not isinstance(evidence, PedestrianIsochroneEvidence):
        raise TypeError("evidence must be a PedestrianIsochroneEvidence")
    ...
    if actual_scales != expected_scales:
        raise ProviderInvariantError("contour evidence must exactly match walking budget policy")
...
def parse_valhalla_isochrone_evidence(
    *,
    ...
    if not isinstance(request, PedestrianIsochroneRequest):
    ...
    warnings = tuple(ValhallaWarningEvidence(warning) for warning in raw_warnings)
    ...
    contours: dict[str, PedestrianContourEvidence] = {}
    ...
    for index, feature in enumerate(features):
        ...
        if geometry_type in {"Polygon", "MultiPolygon"}:
            ...
            if metric not in {None, "time"}:
                raise ProviderInvariantError("canonical pedestrian isochrone accepts time contours only")
            ...
            contours[scale.scale_id] = PedestrianContourEvidence(
    ...
    ordered: list[PedestrianContourEvidence] = []
