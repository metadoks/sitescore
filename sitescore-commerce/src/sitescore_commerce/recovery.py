...
class RecoveryStore:
    ...
    def finish_run(self, response: RecoveryRunResponse, now: datetime) -> None:
        try:
            with self.session_factory.begin() as session:
                row = session.execute(
                    select(RecoveryRunRow).where(RecoveryRunRow.run_id == response.run_id).with_for_update()
                ).scalar_one()
                row.finished_at = now
    ...
    def seed_candidates(self, *, now: datetime, limit: int) -> None:
        """Seed only active orders that do not already have durable recovery state.

        Existing backoff or leased rows therefore cannot consume the seed window and
        starve later active orders.  Existing rows remain eligible directly through
        claim_batch; new rows are admitted oldest-first in a bounded window.
        """
        try:
            with self.session_factory.begin() as session:
                order_ids = session.execute(
                    select(OrderRow.order_id)
                    .outerjoin(RecoveryStateRow, RecoveryStateRow.order_id == OrderRow.order_id)
                    .where(
                        OrderRow.order_state.in_(_ACTIVE_ORDER_STATES),
                        RecoveryStateRow.order_id.is_(None),
                    )
        ...
        try:
            with self.session_factory.begin() as session:
                rows = session.execute(
                    ...
                    .order_by(
                        RecoveryStateRow.next_attempt_at.asc(),
                        RecoveryStateRow.created_at.asc(),
                        RecoveryStateRow.order_id.asc(),
                    )
        ...
        *,
        ...
        try:
            with self.session_factory.begin() as session:
                row = session.execute(
                    select(RecoveryStateRow)
                    .where(RecoveryStateRow.order_id == claim.order_id)
                    .with_for_update()
                ).scalar_one_or_none()
                if (
                    row is None
        ...
        try:
            with self.session_factory.begin() as session:
                row = session.execute(
                    ...
                    .where(OutboxEventRow.outbox_id == event_id, OutboxEventRow.outbox_type == PAID_OUTBOX_TYPE)
                    .with_for_update()
                ).scalar_one_or_none()
                if row is None:
    ...
    def load_snapshot(self, order_id: UUID) -> RecoverySnapshot:
        try:
            with self.session_factory() as session:
                ...
                rows = session.execute(
                    select(OutboxEventRow)
                    .where(OutboxEventRow.order_id == order_id, OutboxEventRow.outbox_type == PAID_OUTBOX_TYPE)
                    .order_by(OutboxEventRow.created_at.asc(), OutboxEventRow.outbox_id.asc())
                ).scalars().all()
                events = tuple(
                    PaidOutboxEvent(row.outbox_id, row.outbox_type, row.order_id, row.created_at)
                    for row in rows
                )
...
class RecoveryService:
    ...
    def _finish_deferred(
        ...
        *,
        ...
        delay = (
            self._backoff(claim.consecutive_failures)
            if failed
            else min(self.runtime.pending_payment_poll_seconds, self.runtime.maximum_backoff_seconds)
        )
    ...
    def _finish_attention(
        ...
        *,
        ...
        if not self.store.lease_is_current(claim, now):
            return
        ...
        self.store.finish_claim(
            ...
            error_code=code,
            failed=True,
            next_delay_seconds=_ATTENTION_DELAY_SECONDS,
        )
    ...
    def _resume_inbox(
        ...
        *,
        ...
        self.commerce_store.apply_reconciliation(
    ...
    def _poll_payment(
        ...
        *,
        ...
        self.store.finish_claim(
            ...
            now=utcnow(),
            action="poll_checkout",
            outcome="reconciled" if outcome == "transitioned" else "deferred",
            error_code=None,
            failed=False,
    ...
    def _process_claim(self, *, claim: RecoveryClaim, run_id: UUID, now: datetime) -> str:
        ...
        stale_event = self.store.load_stale_received_event(
            order_id=claim.order_id,
            cutoff=now - timedelta(seconds=self.runtime.stale_inbox_seconds),
        )
        if stale_event is not None:
            outcome, _ = self._resume_inbox(
                claim=claim,
                run_id=run_id,
                event=stale_event,
                now=now,
        ...
        if snapshot.order_state == OrderState.PENDING_PAYMENT.value and snapshot.payment_state == PaymentState.PENDING.value:
            if snapshot.order_updated_at > now - timedelta(seconds=self.runtime.pending_payment_poll_seconds):
                self._finish_deferred(
                    ...
                    now=now,
                    action="poll_checkout",
                    code="pending_payment_not_stale",
                )
                return "deferred"
            outcome, _ = self._poll_payment(
                claim=claim,
                run_id=run_id,
                snapshot=snapshot,
                now=now,
        ...
        if (
            snapshot.payment_state in {PaymentState.PAID.value, PaymentState.REFUND_PENDING.value}
            and snapshot.order_state in {OrderState.PAID.value, OrderState.FULFILLMENT_IN_PROGRESS.value}
        ):
            ...
            if newest_authority_time > replay_cutoff:
                self._finish_deferred(
                    ...
                    now=now,
                    action="replay_paid_outbox",
                    code="published_order_not_stale",
                )
                return "deferred"
            outcome, _ = self._send_outbox(
                claim=claim,
    ...
    def run_once(self) -> RecoveryRunResponse:
        ...
        self.store.seed_candidates(
            now=started,
            limit=max(self.runtime.batch_size * 4, self.runtime.batch_size),
        )
        claims = self.store.claim_batch(
            now=started,
        ...
        for claim in claims:
            try:
                ...
                now = utcnow()
                if self.store.lease_is_current(claim, now):
                    self._finish_deferred(
                        claim,
                        now=now,
                        action="stripe_provider",
