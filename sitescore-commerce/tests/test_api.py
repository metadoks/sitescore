...
def test_post_orders_requires_idempotency_key():
    r=TestClient(create_app(service=Service())).post("/v1/orders",json=valid_order()); assert r.status_code==400 and r.json()["error"]["code"]=="request_validation_failed"
def test_post_orders_returns_pending_resource_without_stripe_session_id():
    r=TestClient(create_app(service=Service())).post("/v1/orders",headers={"Idempotency-Key":"abc"},json=valid_order()); body=r.json(); assert r.status_code==201; assert body["order_state"]=="pending_payment" and body["payment_state"]=="pending" and body["fulfillment_state"]=="not_started"; assert "stripe_checkout_session_id" not in body and "canonical_request_hash" not in body
def test_strict_body_rejects_paid_forgery():
    p=valid_order(); p["paid"]=True; assert TestClient(create_app(service=Service())).post("/v1/orders",headers={"Idempotency-Key":"abc"},json=p).status_code==422
def test_idempotency_conflict_is_sanitized():
    r=TestClient(create_app(service=Service(IdempotencyConflict("internal hash secret")))).post("/v1/orders",headers={"Idempotency-Key":"abc"},json=valid_order()); assert r.status_code==409; assert "internal hash secret" not in r.text
def test_persistence_error_is_sanitized():
    r=TestClient(create_app(service=Service(PersistenceUnavailable("postgresql://user:password@db")))).post("/v1/orders",headers={"Idempotency-Key":"abc"},json=valid_order()); assert r.status_code==503; assert "password" not in r.text
