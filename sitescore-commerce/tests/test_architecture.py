...
def test_runtime_dependencies_are_exact_and_exclude_embedded_future_systems():
    ...
    # Postmark uses the existing exact httpx transport rather than a new SDK dependency.
...
def test_migrations_only_target_commerce_schema():
    for name in ["0001_commerce_order_checkout.py","0002_webhook_payment_authority.py","0003_fulfillment_refund.py","0004_delivery_email.py"]:
...
def test_6_4_runtime_uses_http_boundaries_not_frozen_tables_or_private_storage():
    text="\n".join(p.read_text() for p in (ROOT/"src").rglob("*.py")).lower()
    # FAZ 6.4 is explicitly authorized to call the frozen HTTP report/content API
    # and Postmark. It still may not import/access frozen DB/runtime/storage internals.
