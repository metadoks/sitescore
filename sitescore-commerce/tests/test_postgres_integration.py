...
def test_upgrade_downgrade_base_upgrade_cycle_is_safe():
    ...
    with engine.connect() as conn: assert conn.execute(sa.text("SELECT version_num FROM commerce.alembic_version")).scalar_one()=="0004_delivery_email"
