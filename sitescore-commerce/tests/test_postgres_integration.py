...
def test_migration_schema_tables_and_constraints():
    migrate(); engine=sa.create_engine(DATABASE_URL); insp=sa.inspect(engine); assert {"orders","order_idempotency","checkout_sessions","stripe_event_inbox","outbox_events","alembic_version"}<=set(insp.get_table_names(schema="commerce")); assert "alembic_version" not in insp.get_table_names(schema="public")
...
def test_upgrade_downgrade_base_upgrade_cycle_is_safe():
    cfg=alembic_config(); command.upgrade(cfg,"head"); command.downgrade(cfg,"base"); engine=sa.create_engine(DATABASE_URL); insp=sa.inspect(engine); assert "commerce" in insp.get_schema_names(); assert set(insp.get_table_names(schema="commerce"))=={"alembic_version"}; command.upgrade(cfg,"head"); insp=sa.inspect(engine); assert {"orders","order_idempotency","checkout_sessions","stripe_event_inbox","outbox_events","alembic_version"}<=set(insp.get_table_names(schema="commerce")); assert "alembic_version" not in insp.get_table_names(schema="public")
    with engine.connect() as conn: assert conn.execute(sa.text("SELECT version_num FROM commerce.alembic_version")).scalar_one()=="0002_webhook_payment_authority"
