...
def expected_tables():
    return {"orders","order_idempotency","checkout_sessions","stripe_event_inbox","outbox_events","fulfillment_bindings","refund_eligibility","refund_operations","delivery_grants","delivery_attempts","recovery_runs","recovery_state","payment_poll_receipts","outbox_replay_audit","recovery_findings","alembic_version"}
def test_migration_schema_tables_and_constraints():
    migrate(); engine=sa.create_engine(DATABASE_URL); insp=sa.inspect(engine); assert expected_tables()<=set(insp.get_table_names(schema="commerce")); assert "alembic_version" not in insp.get_table_names(schema="public")
    ...
    recovery_cols={c["name"] for c in insp.get_columns("recovery_state",schema="commerce")}; assert {"attempt_count","consecutive_failures","next_attempt_at","lease_token","lease_expires_at","last_action","last_outcome","last_error_code"}<=recovery_cols
    receipt_cols={c["name"] for c in insp.get_columns("payment_poll_receipts",schema="commerce")}; assert {"source","stripe_checkout_session_id","observed_session_status","observed_payment_status","observed_payment_intent_id","observed_livemode","evidence_sha256","transition_target","observed_at"}<=receipt_cols
...
def test_upgrade_downgrade_base_upgrade_cycle_is_safe():
    cfg=alembic_config(); command.upgrade(cfg,"head"); command.downgrade(cfg,"base"); engine=sa.create_engine(DATABASE_URL); insp=sa.inspect(engine); assert "commerce" in insp.get_schema_names(); assert set(insp.get_table_names(schema="commerce"))=={"alembic_version"}; command.upgrade(cfg,"head"); insp=sa.inspect(engine); assert expected_tables()<=set(insp.get_table_names(schema="commerce")); assert "alembic_version" not in insp.get_table_names(schema="public")
    with engine.connect() as conn: assert conn.execute(sa.text("SELECT version_num FROM commerce.alembic_version")).scalar_one()=="0005_recovery_reconciliation"
